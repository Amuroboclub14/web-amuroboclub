...
const COLLECTION = "technoxian_11_project_recruitment";
const STATUSES = [
  "pending",
  "reviewed",
  "shortlisted",
...
function ApplicationModal({ application, onClose, onStatusChange, onRefresh }) {
  ...
  const fields = [
    ...
    { key: "contribution", label: "Contribution (one liner)" },
        ...
        <div className="p-4 overflow-y-auto space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm font-mono text-white focus:border-orange-500 focus:outline-none"
            >
              {STATUSES.map((s) => (
...
function TechnoxianApplicationsContent() {
  ...
  const yearOptions = useMemo(() => {
    return Array.from(
      new Set(applications.map((a) => a.yearOfStudy).filter(Boolean)),
  ...
  const filteredApplications = useMemo(() => {
    ...
    if (yearFilter !== "all") {
      list = list.filter((a) => a.yearOfStudy === yearFilter);
    }
    if (searchQuery.trim()) {
  ...
  }, [applications, statusFilter, yearFilter, searchQuery]);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `technoxian-11-applications-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 font-mono">
            Loading Technoxian applications...
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="relative flex-1 min-w-[200px] max-w-xs">
            ...
            <input
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-9 pr-3 py-2 text-sm font-mono text-white placeholder-gray-500 focus:border-orange-500 focus:outline-none"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm font-mono text-white focus:border-orange-500 focus:outline-none"
            >
                ...
                >
                  <td className="py-3 px-2">
                    <button
                      ...
                      className="text-left font-mono text-orange-400 hover:text-orange-300 hover:underline"
                    >
                      {app.name || "—"}
                    </button>
                  </td>
                  ...
                  <td className="py-3 px-2 text-sm font-mono text-gray-300">
                    {app.branch === "Other"
                      ? app.branchOther || "Other"
                      : app.branch || "—"}
                  </td>
                  <td className="py-3 px-2 text-sm font-mono text-gray-300 hidden xl:table-cell">
                    {app.yearOfStudy || "—"}
                  </td>
                  <td className="py-3 px-2 text-sm font-mono text-gray-300 hidden xl:table-cell max-w-[220px] truncate">
                    {app.contribution || "—"}
                  </td>
                  <td className="py-3 px-2">
                    <select
...
export default function TechnoxianApplicationsPage() {
  return (
    <ProtectedRoute>
      <TechnoxianApplicationsContent />
    </ProtectedRoute>
  );
}
